...
class TextToSelect extends UtilityCommand
{
    ...
    {
        $this->setName('attribute-migration:text-to-select')->setDescription('Migrating an Text-Attribute to another Select-Attribute');
    ...
    public function executeCommand()
    {
        ...
        if ($oldAttribute->getFrontendInput() != 'text' || $newAttribute->getFrontendInput() != 'select') {
        ...
        $this->walkProgress($productCollection, function ($product) use ($oldAttribute, $newAttribute) {
            ...
            $oldValue = $product->getData($oldAttribute->getAttributeCode());
            $newValue = $product->getData($newAttribute->getAttributeCode());

            if (empty($oldValue)) {
                return $product->getSku() . ' (nothing to migrate)';
            }
            ...
            $newValueId = $this->attributeUtility->createOrGetOptionId($newAttribute->getAttributeCode(), $oldValue);
